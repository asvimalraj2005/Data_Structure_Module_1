...
def numSetBits(A):
    ...
    Count_of_Set_Bits=0                                 # Using an counter named Count_of_Set_bits for counting the set bits in binaryformat varaible
    for bit in binaryformat:                            # Executing a loop by using bit as the iterator over the binaryformat variable 
        if bit=='1':                                    # if bit=='1' then we are incrementing Count_of_Set_Bits by 1   
